    ...
    if fixed_avg_type == OptionAvgType.ARITHMETIC:
        ...
        if not analytics_mode:
            df['numeric_avg_type'] = df.apply(lambda row: 1 if row.avg_type == OptionAvgType.ARITHMETIC.value else 0,
                                              axis=1)
            val_df['numeric_avg_type'] = val_df.apply(lambda row: 1 if row.avg_type == OptionAvgType.ARITHMETIC.value else 0,
                                                      axis=1)
        if WITH_CI_STATS:
            df_values = df[['spot_strike_ratio', 'ttm', 'risk_free_rate', 'volatility', 'numeric_avg_type', 'left_ci', 'right_ci']].astype(
                np.float32).to_numpy()
            val_df_values = val_df[['spot_strike_ratio', 'ttm', 'risk_free_rate', 'volatility', 'numeric_avg_type', 'left_ci',
                            'right_ci']].astype(
                np.float32).to_numpy()
        else:
            df_values = df[['spot_strike_ratio', 'ttm', 'risk_free_rate', 'volatility', 'numeric_avg_type']].astype(
                np.float32).to_numpy()
            val_df_values = val_df[['spot_strike_ratio', 'ttm', 'risk_free_rate', 'volatility', 'numeric_avg_type']].astype(
                np.float32).to_numpy()
