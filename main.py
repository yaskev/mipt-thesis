...
def make_predicted_df(x_test: list, y_test: list, predict_price: np.ndarray, fixed_avg_type: OptionAvgType = None):
    ...
    df_predicted['time_value'] = predict_price
...
def main():
    ...
    df_test = remove_time_value_column(df_test)
    df_test['monte_carlo_price'] += (df_test['net_price'] - df_test['time_value'])
    df_test.to_csv('con_time_prices.csv' if USE_CONVEX_NETWORK else 'pos_time_prices.csv', index=False,
                   float_format='%.4f')
    print(f'MSE: {((df_test["monte_carlo_price"] - df_test["net_price"]) ** 2).mean()}')
