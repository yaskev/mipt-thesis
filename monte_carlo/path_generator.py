...
from settings import NUMBER_OF_PATHS, NUMBER_OF_STEPS, USE_ANTITHETIC_VARIATES, PATH_RANDOM_SEED, TIME_TO_MATURITY


def generate_paths(risk_free_rate: float,
                   volatility: float,
    ...
    return create_paths(normal_samples, risk_free_rate, volatility, spot_strike_ratio)


@njit(fastmath=True)
def create_paths(paths: np.ndarray,
                 risk_free_rate: float,
                 volatility: float,
                 spot_strike_ratio: float):
    paths[:, 0] = spot_strike_ratio
    dt = TIME_TO_MATURITY / NUMBER_OF_STEPS
