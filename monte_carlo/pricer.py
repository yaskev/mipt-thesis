...
from settings import USE_ANTITHETIC_VARIATES, NUMBER_OF_PATHS, CONFIDENCE_INTERVAL, TIME_TO_MATURITY
...
def get_option_price_and_ci(paths: np.ndarray,
    ...
    payoff_mean = np.exp(-risk_free_rate * TIME_TO_MATURITY) * payoffs.mean()
    payoff_std = np.exp(-risk_free_rate * TIME_TO_MATURITY) * payoffs.std()
